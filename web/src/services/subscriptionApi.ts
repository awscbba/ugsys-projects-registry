...
export const subscriptionApi = {
  ...
  getMySubscriptions(personId: string): Promise<Subscription[]> {
    return httpClient.get<Subscription[]>(`/api/v1/subscriptions/person/${personId}`);
  },

  publicCheckEmail(email: string): Promise<{ exists: boolean }> {
    return httpClient.post<{ exists: boolean }>('/api/v1/public/check-email', { email });
  },
  ...
  publicSubscribe(data: {
    ...
    return httpClient.post<{ subscription_id: string }>('/api/v1/public/subscribe', data);
  },
